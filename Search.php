            ...
            <div class="all-blog-posts">
              <div class="row">
                   <?php
                    if(isset($_POST['sub'])){
					$search= $_POST['search'];
					$query="SELECT * FROM blogs WHERE blog_keywords LIKE '%$search%' ";
					$query_seach=mysqli_query($connection,$query);
					if(!$query_seach){
					die("no" . mysqli_error($connection));
					}
					$count=mysqli_num_rows($query_seach);
					if($count==0){
					echo "<h1>No result</h1>";
				} else{
				while($row=mysqli_fetch_assoc($query_seach)){
					$blog_id=$row['blog_id'];
					...
					$cont_short=substr($row['blog_content'],0,50);
